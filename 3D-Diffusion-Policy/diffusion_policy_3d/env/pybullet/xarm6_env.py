...
class XArm6Robotiq85:
    ...
    def load(self):
        self.id = p.loadURDF(
            "/home2/darshil.jariwala/3D-Diffusion-Policy/3D-Diffusion-Policy/diffusion_policy_3d/env/pybullet/urdf/xarm6_robotiq_85.urdf",
            self.base_pos,
            self.base_ori,
            useFixedBase=True,
        )
...
class XArm6PickPlaceEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.cube_id is not None:
            ...
            # Success condition:
            # 1. Cube within cylinder radius (XY plane)
            # 2. Cube lifted above cylinder top
            # 3. Gripper is open (released)
            if (dist_xy < cyl_radius and 
                cube_pos[2] > cyl_top_z + 0.01 and 
                self.robot.get_gripper_normalized() < 0.2):
                reward = 1.0
