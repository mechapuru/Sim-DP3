"""
XArm6 Gripper (Lite6) Gym Environment

A gym.Env wrapper for the Lite6 robot + custom gripper simulation.
Actions are absolute joint angles (6D) + gripper delta (1D).

Gripper delta logic (Prismatic):
  - delta > 0.1  → close gripper (target 0.0)
  - delta < -0.05 → open gripper (target -0.04)
...
# ─────────────────────────────────────────────
CUBE_LENGTH = 0.05
# Lite6 Prismatic Gripper:
# Open = -0.04 (fingers apart)
# Closed = 0.0 (fingers touch)
# Grasp ~ -0.024 (for 5cm box)
GRIPPER_CLOSE_TARGET = 0.0      
GRIPPER_OPEN_TARGET = -0.04     
GRIPPER_MAX_ANGLE = 0.04       # For normalization (approx range size)
...
class Lite6Robot:
    """Lite6 Robot with Custom Prismatic Gripper"""
    ...
    def load(self):
        # Note: Using the updated URDF path for Lite6
        self.id = p.loadURDF(
            "/home2/darshil.jariwala/3D-Diffusion-Policy/3D-Diffusion-Policy/diffusion_policy_3d/env/pybullet/lite-6-updated-urdf/lite_6_new.urdf",
            self.base_pos,
            self.base_ori,
            useFixedBase=True,
        )
    ...
    def _setup_mimic_joints(self):
        ...
        try:
            ...
            raise ValueError(f"Could not find mimic parent joint '{mimic_parent_name}'")
    ...
    def get_gripper_normalized(self):
        """
        Return gripper state normalized to [0, 1].
        Open (-0.04) -> 0.0
        Closed/Grasp (-0.024 or higher) -> 1.0 (clamped)
        """
        raw = p.getJointState(self.id, self.mimic_parent_id)[0]
        
        open_pos = -0.04
        grasp_pos = -0.024 # Target close position for 5cm box (approx)
        
        if abs(grasp_pos - open_pos) < 1e-5:
            return 0.0
            
        norm = (raw - open_pos) / (grasp_pos - open_pos)
        return min(max(norm, 0.0), 1.0)

    def get_gripper_raw_angle(self):
        """Return raw gripper left_finger_joint position (meters)."""
    ...
    def command_gripper_close(self, target_pos=GRIPPER_CLOSE_TARGET, force=500):
        ...
        for jid, mult in self.mimic_child_multiplier.items():
            p.setJointMotorControl2(
                self.id, jid, p.POSITION_CONTROL,
                targetPosition=target_pos * mult, force=force,
            )

    def command_gripper_open(self, target_pos=GRIPPER_OPEN_TARGET, force=500):
    ...
    def reset_posture(self):
        ...
        p.setJointMotorControl2(
            self.id, self.mimic_parent_id, p.POSITION_CONTROL,
            targetPosition=gripper_open_val, force=500,
        )
        for jid, mult in self.mimic_child_multiplier.items():
            p.resetJointState(self.id, jid, gripper_open_val * mult, targetVelocity=0)
            p.setJointMotorControl2(
                self.id, jid, p.POSITION_CONTROL,
                targetPosition=gripper_open_val * mult, force=500,
            )
...
class XArm6GripperEnv(gym.Env):
    """
    PyBullet Lite6 Pick-and-Place Environment (formerly XArm6Gripper).
    ...
    def __init__(
        ...
        cprint(f"XArm6GripperEnv initialized — action_dim={action_dim}, "
        ...
        self.tp_cam_eye = [0.7463, 0.3093, 0.62 + 0.5574]
    ...
    def _apply_gripper_delta(self, gripper_delta):
        """
        Apply gripper delta logic for Lite6 Prismatic Gripper:
          delta > 0.1   → close (target 0.0)
          delta < -0.05 → open (target -0.04)
          else          → hold
        """
        if gripper_delta > GRIPPER_CLOSE_DELTA_THRESH:
            # Close: keep sending close commands
            max_iters = 100
            for _ in range(max_iters):
                ...
                # Stop if we hit 1.0 normalized or raw > -0.024 (closed enough/grasped)
            ...
            for _ in range(max_iters):
                p.stepSimulation()
                raw_pos = self.robot.get_gripper_raw_angle()
                # Stop if almost fully open (-0.039)
                if raw_pos <= -0.039:
                    break
        # else: hold — do nothing (friction keeps it in place usually)
